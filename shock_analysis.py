...
#-------------------------------------------------------------------------------
# FUNCTIONS AND PROCEDURES REQUIRED BY THE SHOCK_ANALYSIS.PRO
#-------------------------------------------------------------------------------

# This function is used to check that there are atleast three datapoints in the dataframe
# in both the upstream and downstream intervals
def analysis_interval_check(t_up, t_down, var_names, df):
   
    # Initializing the returned boolean
    too_few_datapoints = False

    # Looping through the given variables
    for var in var_names:

        # Choosing the data in the upstream and downstream intervals
        data_up = df[(df['EPOCH'] >= t_up[0]) & (df['EPOCH'] <= t_up[1])][var]
        data_down = df[(df['EPOCH'] >= t_down[0]) & (df['EPOCH'] <= t_down[1])][var]

        # Counting the datapoints in the intervals
        points_up = data_up.notna().sum()
        points_down = data_down.notna().sum()

        # If less than three datapoints either in up or downstream
        # return true
...
# This function returns the resolution of the given variable
# for a spacecraft.
def return_resolution(var_name, SC, Helios_flag):
    ...
    # Return the resolution corresponding the the input variable
    res = mag_res if var_name.startswith('B') else pla_res

    return res


# This function calculates the mean and the standard deviation
# for the given variables in var_names in the given time interval and 
# saves them to the corresponding dictionary (mean value dictionary and std value dictionary)
def mean_data(var_names, interval, data_dict):
    ...
    for var in var_names:
        if var in data_dict:
            data = data_dict[var]
            data_interval = data_dict[(data_dict['EPOCH'] >= t_start) & (data_dict['EPOCH'] <= t_end)][var]
...
def calculate_mean_resolution(t_up, t_down, var_names, SC, Helios_flag, data_dict):
    ...
    for var in var_names:
        if var in data_dict:
            ...
            data_interval = data_dict[(data_dict['EPOCH'] >= interval_start) & 
                          (data_dict['EPOCH'] <= interval_end) & 
                          data_dict[var].notna()][var]
            
            gd_points = data_interval.count()  # Count non-null values after applying the mask
...
# This function is used to interpolate var_data to lower resolution, form t_original to
# t_lower res time
def resample(var_data, t_original, t_lower_res, interval, SC_ID, bin_rad=None):

    # Count the number of time ticks in the lower res series within the interval
    interval_mask = np.where((t_lower_res >= interval[0]) & (t_lower_res <= interval[1]))[0]
    ...
    ind_pla = np.concatenate(([interval_mask[0]-1], interval_mask, [interval_mask[-1]+1]))
    
    diffs = np.abs(np.diff(t_lower_res[ind_pla]))  # Calculate differences between the points in t_lower_res
    min_res = np.min(diffs[1:-1])
    
    # For Wind SC there's a different radius for each data point
    if SC_ID == 1:
        bin_rad_up = bin_rad[interval_mask]
    else:
        bin_rad_up = 0

    # Determine measurement bin radiuses
    ...
    if bin_rad is not None:
        for i in range(n_interval_ticks):
            ...
            if SC_ID in [11, 12] and local_min_res / 2 > rad: #Special case for Voyager SCs
                rad = local_min_res / 2

            t_bins = np.array([[time - rad, time + rad] for time in interval_times])
    else:
        for i in range(n_interval_ticks):
            t_bins = np.array([[time - min_res / 2, time + min_res / 2] for time in interval_times])

    # Initialize array where averaged values will be added
    resampled_dat = np.zeros(n_interval_ticks)

    # Looping through the bins and averaging the data
    for j in range(n_interval_ticks):
        # Find indices of t_original that are within the current bin range
        indices = np.where((t_original > t_bins[j, 0]) & (t_original <= t_bins[j, 1]))[0]

        if len(indices) > 0:
            # Average values in var_data corresponding to these indices
            resampled_dat[j] = np.nanmean(var_data[indices])
        else:
            # If no indices fall within the bin, assign NaN
            resampled_dat[j] = np.nan
...
shock_times_out_fnam = 'shocks_out.dat' # If shock times are preliminary, 
                                        # better estimates are saved here.

analysis_output_fnam = 'shock_parameters.dat'   # Output file of the analysis 
                                                # results
...
with open(shock_times_fnam, 'r') as file:
    ...
    for i in range(11, len(lines)):
        ...
        datetime_format = datetime(int(year), int(month), int(day), int(hour), int(minute), int(second))
        shock_datetimes.append(datetime_format)
        shock_formatted_times.append(datetime_format.strftime('%Y-%m-%d/%H:%M'))

    t_shock = [(dt - datetime(1970, 1, 1)).total_seconds() for dt in shock_datetimes] # Changes time format to seconds since Jan 1, 1970.
...
#-------------------------------------------------------------------------------
    ...
    mag_dataframe, pla_dataframe, SC_pos, output_add, t_shock_new, pla_bin_rads = download_and_process_data(shock_datetimes[i], SC_ID, filter_options)
...
#-------------------------------------------------------------------------------
    ...
    for j in range(1, -1, -1):
        ...
        # Other SCs
        if (SC_ID != 4) and (SC_ID !=  5) and (SC_ID !=  6) and (SC_ID !=  11) and (SC_ID !=  12):
        ...
        if (SC_ID == 4) or (SC_ID ==  5) or (SC_ID ==  6) or (SC_ID ==  11) or (SC_ID ==  12):
            while abs(upstream_furthest - upstream_gap) <= length_limit:
                ...
                too_few_mag_points = analysis_interval_check(t_up, t_down, mag_vars, mag_dataframe)
                too_few_pla_points = analysis_interval_check(t_up, t_down, pla_vars, pla_dataframe)
                ...
                if too_few_mag_points or too_few_pla_points:
                    if ((SC_ID == 4) or (SC_ID == 5)) and (Helios_dataset_flag == 0) and (too_few_mag_points == 1):
                    ...
                    if too_few_pla_points == 0:
                        increment = return_resolution(mag_vars[0], SC_ID, Helios_dataset_flag)
                    else:
                        increment = return_resolution(pla_vars[0], SC_ID, Helios_dataset_flag)
        ...
        if ((too_few_mag_points == 1) or (too_few_pla_points == 1)) and (j == 1):
            continue
        
        # If FR-type check does not contains enough data points, reject the whole
        # event
        jump_to_plotting = False
        if ((too_few_mag_points == 1) or (too_few_pla_points == 1)) and (j == 0):
        ...
        B_vector_up = [mag_up_dct[0]["Bx"], mag_up_dct[0]["By"], mag_up_dct[0]["Bz"]]
        B_vector_down = [mag_down_dct[0]["Bx"], mag_down_dct[0]["By"], mag_down_dct[0]["Bz"]]
        ...
        V_vector_up = [pla_up_dct[0]["Vx"], pla_up_dct[0]["Vy"], pla_up_dct[0]["Vz"]]
        V_vector_down = [pla_down_dct[0]["Vx"], pla_down_dct[0]["Vy"], pla_down_dct[0]["Vz"]]
...
#-------------------------------------------------------------------------------
    ...
    # bulk speed V. USED ONLY FOR STEREO A AND B SPACECRAFT
        ...
        df_up = pla_dataframe[(pla_dataframe['EPOCH'] >= t_up[0]) & (pla_dataframe['EPOCH'] <= t_up[1])]
        df_down = pla_dataframe[(pla_dataframe['EPOCH'] >= t_down[0]) & (pla_dataframe['EPOCH'] <= t_down[1])]
    ...
    #------------------------------------------------------------------------------#
        ...
        avg_res_mag = calculate_mean_resolution(t_up, t_down, mag_vars, SC_ID, Helios_dataset_flag, mag_dataframe)
        avg_res_pla = calculate_mean_resolution(t_up, t_down, pla_vars, SC_ID, Helios_dataset_flag, pla_dataframe)
    ...
    #-------------------------------------------------------------------------------
        ...
        Np = pla_dataframe[(pla_dataframe['EPOCH'] >= t_up[0]) & (pla_dataframe['EPOCH'] <= t_up[1])]['Np']
        Tp = pla_dataframe[(pla_dataframe['EPOCH'] >= t_up[0]) & (pla_dataframe['EPOCH'] <= t_up[1])]['Tp']
        V = pla_dataframe[(pla_dataframe['EPOCH'] >= t_up[0]) & (pla_dataframe['EPOCH'] <= t_up[1])]['V']
        Bt = mag_dataframe[(mag_dataframe['EPOCH'] >= t_up[0]) & (mag_dataframe['EPOCH'] <= t_up[1])]['B']
        
        # Magnetic field data is resampled to the resolution of plasma data if 
        # B-data has higher resolution
        if (SC_ID in [0,1,2,3,6,11,12,13,15]) or (SC_ID in [4,5] and Helios_dataset_flag == 0):

            B_rs = resample(mag_dataframe['B'], mag_dataframe['EPOCH'], pla_dataframe['EPOCH'], t_up, SC_ID, bin_rad)
            B_is_averaged = 1

        # PSP - plasma data is higher res than mag data
        elif SC_ID in [14]:
            Np = resample(pla_dataframe['Np'], pla_dataframe['EPOCH'], mag_dataframe['EPOCH'], t_up, SC_ID, bin_rad)
            Tp = resample(pla_dataframe['Tp'], pla_dataframe['EPOCH'], mag_dataframe['EPOCH'], t_up, SC_ID, bin_rad)
            V = resample(pla_dataframe['V'], pla_dataframe['EPOCH'], mag_dataframe['EPOCH'], t_up, SC_ID, bin_rad)
            B_rs = Bt
            ...
            if (SC_ID == 7) or (SC_ID == 8) or (SC_ID == 9):
                B_rs = np.interp(pla_dataframe[(pla_dataframe['EPOCH'] >= t_up[0]) & (pla_dataframe['EPOCH'] <= t_up[1])]['EPOCH'], mag_dataframe['EPOCH'], mag_dataframe['B'])
                
            else:
                B_rs = mag_dataframe[(mag_dataframe['EPOCH'] >= t_up[0]) & (mag_dataframe['EPOCH'] <= t_up[1])]['B']
                B_is_averaged = 0
        ...
        if (SC_ID == 4) or (SC_ID == 5) or (SC_ID == 6) or (SC_ID == 11) or (SC_ID == 12) or (SC_ID == 14) or (SC_ID == 15):
        ...
        pla_char_up_dct = mean_data(['Cs','V_A','Vms','beta','Dp'], t_up, pla_char_dataframe)

        # For error analysis purposes calculate also other means
        # (basically partial derivatives)
        doo_Cs_doo_Te = kB / (2 * mp) * np.nanmean((1000. * pla_char_dataframe['Cs']) ** (-1)) / 1000
        doo_Vms_doo_Te = kB / (2 * mp) * np.nanmean((1000. * pla_char_dataframe['Vms']) ** (-1)) / 1000
    ...
    #method IDs: 0=MX3-method,1=MFC-method,2=MX1+MX2-avg-method,3=MVA-analysis
        ...
        normal = shock_normal(type, B_vector_up, B_vector_down, V_vector_up, V_vector_down, normal_method_ID)
        ...
        if (np.dot(delta_B, normal) > 1e-12 or
                np.dot(np.cross(delta_B, delta_V), normal) > 1e-12) and normal_method_ID == 0:
            raise ValueError('MX3-normal is not correct: should be perp to (delta_V x delta_B) and delta_B')
    ...
    #-------------------------------------------------------------------------------
        ...
        CosTheta = abs(np.dot(B_vector_up, normal)) / (np.linalg.norm(B_vector_up) * np.linalg.norm(normal))
    ...
    #-------------------------------------------------------------------------------
        ...
        errors = error_analysis(mag_up, mag_up_std, mag_down, mag_down_std, pla_up,
                                pla_up_std, pla_down, pla_down_std, char_up,
                                char_std_up, Te_std, doo_Cs_doo_Te, doo_Vms_doo_Te,
                                doo_beta_doo_Te, V_shock, M_A_up, Mms_up, normal,
                                normal_method_ID, shock_type)
#-------------------------------------------------------------------------------
#Drawing a ps.-plots in a file (named e.g. '19980205_2105.ps') 
#-------------------------------------------------------------------------------

#   jumpping to here if jump_to_plotting was set to true

    month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec']
    SC_names = ['ACE', 'Wind', 'STEREOA', 'STEREOB', 'HeliosA', 'HeliosB', 'Ulysses', 'Cluster3', 'Cluster1', 'Cluster4', 'OMNI', 'Voyager1', 'Voyager2', 'DSCOVR', 'PSP', 'SolO']
    SC_titles = ['ACE', 'Wind', 'STEREO A', 'STEREO B', 'Helios A', 'Helios B', 'Ulysses', 'Cluster 3', 'Cluster 1', 'Cluster 4', 'OMNI', 'Voyager 1', 'Voyager 2', 'DSCOVR', 'PSP', 'SolO']
    ...
    mag_dataframe['EPOCH'] = [datetime.fromtimestamp(element) for element in mag_dataframe["EPOCH"]]
    pla_dataframe['EPOCH'] = [datetime.fromtimestamp(element) for element in pla_dataframe["EPOCH"]]

    # Adjust time range based on SC_ID
    if SC_ID in [4, 5, 6, 11, 12]:
        time_range = [shock_time - timedelta(hours=1), shock_time + timedelta(hours=1)]
    else:
        time_range = [shock_time - timedelta(minutes=30), shock_time + timedelta(minutes=30)]

    # Start of your plotting code
    fig, axs = plt.subplots(4, 1, figsize=(12, 10))

    # Example plot, replace with actual data and plots
    axs[0].plot(mag_dataframe['EPOCH'].values, mag_dataframe['B'].values, label='B [nT]', color='black', linewidth=0.6, markersize=9)
    axs[0].set_ylabel('B [nT]', fontsize=12)
    axs[0].set_xlim(time_range)
    axs[0].tick_params(axis='both', which='both', direction='in', bottom=True, top=True, left=True, right=True, labelsize=12)
    ...
    axs[1].plot(pla_dataframe['EPOCH'].values, pla_dataframe['V'].values, label='V [km/s]', marker='+', color='black', linewidth=0.6, markersize=9)
    axs[1].set_ylabel('V [km/s]', fontsize=12)
    axs[1].set_xlim(time_range)
    axs[1].tick_params(axis='both', which='both', direction='in', bottom=True, top=True, left=True, right=True, labelsize=12)
    axs[1].xaxis.set_minor_locator(AutoMinorLocator(10))
    axs[1].yaxis.set_minor_locator(AutoMinorLocator(5))
    axs[1].tick_params(labelbottom=False)

    axs[2].plot(pla_dataframe['EPOCH'].values, pla_dataframe['Np'].values, label='Np [1/cm^3]', marker='+', color='black', linewidth=0.6, markersize=9)
    axs[2].set_ylabel('Np [1/cm^3]', fontsize=12)
    axs[2].set_xlim(time_range)
    axs[2].tick_params(axis='both', which='both', direction='in', bottom=True, top=True, left=True, right=True, labelsize=12)
    axs[2].xaxis.set_minor_locator(AutoMinorLocator(10))
    axs[2].yaxis.set_minor_locator(AutoMinorLocator(5))
    axs[2].tick_params(labelbottom=False)


    axs[3].plot(pla_dataframe['EPOCH'].values, pla_dataframe['Tp'].values, label='Tp [K]', marker='+', color='black', linewidth=0.6, markersize=9)
    axs[3].set_ylabel('Tp [K]', fontsize=12)
    axs[3].set_xlim(time_range)
    axs[3].ticklabel_format(axis='y', style='sci', scilimits=(0, 0))
    axs[3].tick_params(axis='both', which='both', direction='in', bottom=True, top=True, left=True, right=True, labelsize=12)
    ...
    for ax in axs:
        ...
        ax.axvline(datetime.fromtimestamp(t_down[0]), color='black', linestyle='--', linewidth=1, dashes=[8, 8])
        ax.axvline(datetime.fromtimestamp(t_down[1]), color='black', linestyle='--', linewidth=1, dashes=[8, 8])
        ax.axvline(datetime.fromtimestamp(t_up[0]), color='black', linestyle='--', linewidth=1, dashes=[8, 8])
        ax.axvline(datetime.fromtimestamp(t_up[1]), color='black', linestyle='--', linewidth=1, dashes=[8, 8])
    ...
    plt.gcf().text(0.03, 0.06, f'{text_time}\n{text_date}', ha='left', fontsize=13, weight='light')

    # Adding sideways text to the right side of the bottom plot
    side_text = shock_time.strftime('%a %b %d %H:%M:%S %Y')
    axs[3].text(1.005, 0.305, side_text, ha='left', va='center', rotation=90, fontsize=6, transform=axs[3].transAxes)
    ...
    t_print = time.strftime("%Y    %#m    %#d    %#H    %#M    %#S", time.localtime(t_shock[i]))
...
#-------------------------------------------------------------------------------
    ...
    print('Time left (estimate): ' + str(duration_minutes) + ' minutes ' + str(duration_seconds) + ' seconds.')

    
#print out bad events
if len(bad_events) > 0:

    print('There was not enough data to do proper analysis for the following events:')
